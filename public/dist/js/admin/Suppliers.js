...
function GetSuppliers  () {
    ...
    var table = $('#tbl-Suppliers').DataTable();
...
console.log(obj);
        for (var i =  0 ; i < output.length; i++){
            ...
            // if (output[i].SupplierType == 0){
            //     var SupplierType ='محلى'
            // }else{
            //     var SupplierType ='اجنبى'
            // }


             var text ='<tr><td></td>'
                +'<td>'+output[i].SupplierName+'</td>'
                +'<td>'+output[i].SupplierType+'</td>'
                +'<td>'+output[i].SupplierAccountID+'</td>'
                +'<td>'+output[i].SupplierCommision+'</td>'
                +'<td>'+output[i].Kalamia+'</td>'
                +'<td>'+'<button name="EditSupplier_' + output[i].SupplierID + '" class="btn btn-flat btn-info btn-sm EditSupplier">تعديل</button>' +
                ' ' +
                '<button name="DelSupplier_' + output[i].SupplierID + '" class="btn btn-flat btn-danger btn-sm RmvSupplier">حذف</button>'+'</td>'
                +'</tr>';
    ...
    $.ajax({
        ...
        .done(function (output) {
            ...
            if (output.status) {
                ...
                if (output.data.SupplierType == 0) {
                    temp1 = config.suppliertypein
                } else if (output.data.SupplierType == 1) {
                    temp1 = config.suppliertypeout
                } else {
                    temp1 = ''
                }
                ;

                if (output.data.CollectType == 0) {
                    temp2 = config.collecttypenow
                } else if (output.data.CollectType == 1) {
                    temp2 = config.collecttypeafter
                } else {
                    temp2 = ''
                }
                ;
