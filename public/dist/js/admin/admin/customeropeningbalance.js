...
customeropeningbalance.prototype.Validate = function(frm){
    ...
    if (! data.CustomerID ) {
        errors.push(config1.required_customerid);
    }
...
customeropeningbalance.prototype.getallMountForCusromer = function(){

     var t2 = $("#tbl-ValueOFCustomer").DataTable();


    $.ajax({
            url: "gettotalvalueofopeningbalance",
            type: "post",
...
customeropeningbalance.prototype.postcustomeropeningbalance = function(){
    ...
    var errors = this.Validate($('#customeropeningbalance-form :input').serializeArray());

     x=$('#customeropeningbalance-form :input');
    ...
    if (errors.length > 0){
        ...
        $.ajax({
            url: "customeropeningbalance" ,
            headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
            type: "post",
            data: $('#customeropeningbalance-form :input').serialize().replace(/ *\_[^=]*\= */g, "=") ,
            dataType: "json"
        })
            .done(function (output) {
                if (!output.status){
                    ...
                    $('#customeropeningbalance-form [name=Mount]', $('.box-success')).val('').focus();
                    $('#customeropeningbalance-form #cboCustomerID', $('.box-success')).text("");
                    $('#customeropeningbalance-form #cboCustomerID', $('.box-success')).trigger("chosen:updated");
                    $('#customeropeningbalance-form [name=Notes]', $('.box-success')).val('').focus();
                    // $('#customeropeningbalance-form [name=Debt]', $('.box-success')).val('').focus();
                    $('#customeropeningbalance-form [name=TransDate]', $('.box-success')).val('').focus();
...
customeropeningbalance.prototype.addnewrow = function(id){
  ...
  var t = $('#tbl-CustomerOpeningBalance').DataTable();

     var cboCustomerID = $( "#customeropeningbalance-form #cboCustomerID" ).clone();

    text = '<tr><td></td>' +
                '<td>' + $('#customeropeningbalance-form [name=TransDate]').val() +  '</td>' + 
                '<td>' + $('#customeropeningbalance-form [name=Mount]').val() +  '</td>' + 
                '<td data-val="' + $('#customeropeningbalance-form #cboCustomerID').val() + '"> ' + cboCustomerID.text() +  '</td>' + 
                '<td>' + $('#customeropeningbalance-form [name=Notes]').val() +  '</td>' + 
                '<td>' + deptText +'</td>' + 
                '<td>'
...
customeropeningbalance.prototype.DeleteCustomerOpeningBalance = function(TransID){
    $.ajax({
        url: "customeropeningbalance/" + TransID ,
        headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
        type: "delete",
        dataType: "json"
    })
...
$(document).ready(function() {
_customeropeningbalance.getallMountForCusromer();
 ...
 $( ".datepicker" ).datepicker({
    ...
    $('#add-CustomerOpeningBalance').click(function(){
        // alert("aaaaaaaaaaaaa");
...
//});
 ...
 $(document).ready(function() { 
    var supopening = $('#tbl-CustomerOpeningBalance').DataTable();
