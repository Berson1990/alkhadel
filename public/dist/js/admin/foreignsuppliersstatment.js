...
$(function () {
    _foreignSuppliersstatment = new foreignSuppliersstatment();
});
...
//        //console.log(ex);
    ...
    $.ajax({
        ...
        var obj = eval(output);
        var TotalFooter = 0;
        var finalOtherExpenses = 0;
        var FinalCustomMonut = 0;
        var FinalNowlon = 0;
        var finalfinal = 0;
        var FinalCom = 0
        var finalfinaltotalFS = 0;
        for (var i = 0; i < obj.length; i++) {
            var Total = Math.round(obj[i].Total);
            var b = Math.round(obj[i].CustomMount + obj[i].OtherExpenses + obj[i].Nowlon)
            console.log(b);
            var final = Math.round(obj[i].Total) - b;

            finalcom = Math.round((obj[i].Commision / 100) * obj[i].Total);
//        finalcom=Math.round((Commision/100)*final);
            finaltotalFS = final - finalcom;


            text = '<tr><td>' + obj[i].ContainerIntNum + '</td>' +
                '<td>' + Total + '</td>' +
                '<td>' + obj[i].OtherExpenses + '</td>' +
            ...
            TotalFooter += Total;
            finalOtherExpenses += obj[i].OtherExpenses
            FinalCustomMonut += obj[i].CustomMount
            FinalNowlon += obj[i].Nowlon
            finalfinal += final
            FinalCom += finalcom
            finalfinaltotalFS += finaltotalFS
            table.row.add($(text)).draw();


        }
        $("#Total").text(TotalFooter);
        $("#OtherExpenses").text(finalOtherExpenses);
        $("#CustomeMonut").text(FinalCustomMonut);
        $("#Nowlan").text(FinalNowlon);
        $("#final").text(finalfinal);
        $("#finalCommision").text(FinalCom);
        $("#finalTotalFS").text(finalfinaltotalFS);
...
//				 //console.log(text2);    
        ...
        for (var i = 0; i < obj.length; i++) {
            ...
            t.row.add($(text)).draw();
            finalaftercarrying += Math.round(finaltotal)
            SetIntNum(IntNum);
        }


        $("#firstTotal").text(final1);
        $("#finaltotala2").text(finalaftercarrying);
            ...
            if (checkRadio == 2) {
                ...
                $('#tbl-ForeignSuppliersstatment tbody').find('.group').each(function (i, v) {
                    ...
                    $(this).nextUntil('.group').each(function () {
                        ...
                        T_Weight = T_Weight + parseFloat($(this).find(".sum_weight").text());
                        // T_Weight  = T_Weight);
                        T_Quantity = T_Quantity + parseFloat($(this).find(".sum_quantity").text());
                        T_Quantity = Math.round(T_Quantity);
                        ...
                        T_Crrying = T_Crrying + parseFloat($(this).find(".crrying").text());
                        T_Crrying = Math.round(T_Crrying);
